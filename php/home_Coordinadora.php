	...
	if( $con )
 	{
    	...
    	$sql = "SELECT p.NumeroIdentificacion,p.Nombres,z.Id_Zona,z.Zonas
						...
						$data['html'].='<div class="card">';
						  $data['html'].='<div class="card-body">';
							...
							// traerIntervencionGestora();
								...
								foreach(traerIntervencionGestora() as $datosGestora)
								{
										$data['html'].='<a href="#" class="list-group-item list-group-item-action">'.$datosGestora['Municipio'].'-'.$datosGestora['Fecha_Intervencion'].'-'.$datosGestora['Comportamientos'].' <span class="float-right badge badge-primary">1</span></a>';
								}
...
function traerIntervencionGestora()
{
	include "conexion.php";
	...
	where i.Personas_por_Zona_id_Personas_por_Zonacol = '2' group by c.Comportamientos";
